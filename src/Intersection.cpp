...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle) {
    // Lock mutex to protect print statements and add vehicle to queue safely
    std::unique_lock<std::mutex> lck(_mtx);
    ...
    lck.unlock(); // Unlock as we no longer need to protect shared resources

    // Wait until the vehicle is allowed to enter
    ftrVehicleAllowedToEnter.wait();
    lck.lock(); // Re-lock to safely access shared resources
    std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is granted entry." << std::endl;

    // Check traffic light phase and wait if it's red
    if (_trafficLight.getCurrentPhase() == TrafficLightPhase::red) {
        std::cout << "Intersection #" << _id << ": Traffic light is red. Vehicle #" << vehicle->getID() <<
                " is waiting." << std::endl;
        _trafficLight.waitForGreen();
        std::cout << "Intersection #" << _id << ": Traffic light is green. Vehicle #" << vehicle->getID() <<
                " can enter now." << std::endl;
    }

    lck.unlock(); // Ensure we don't hold the lock longer than necessary
}
...
void Intersection::simulate() {
    _trafficLight.simulate(); // Start the traffic light simulation
